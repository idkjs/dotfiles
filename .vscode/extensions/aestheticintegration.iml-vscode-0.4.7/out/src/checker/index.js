...
class DocState {
    ...
    updateEditor() {
        if (this.editor) {
            ...
            if (this.debug)
                console.log(`send doc_check for ${this.uri}:${this.version}`);
...
async function launch(ctx) {
    ...
    console.log(`imandra.debug: ${config.debug}, persistent cache: ${config.persistentCache}, auto update: ${config.autoUpdate}`);
