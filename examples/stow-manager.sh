...
# https://github.com/samba2/stow-manager
########################################################################
#
#  Stow-Manager
#
...
function updatePackageList {
    repositoryFile=$(getRepositoryFilePath)
...
function installPackage {
    ...
    packageName=$(getPackageNameToInstall "${packageParam}")
    local packageName
...
function upgradePackage {
    ...
    if isValidPackageName "${packageParam}"; then
        component=$(getComponentNameFromPackage "${packageParam}")
        local component
    ...
    if [ ! "${newPackage}" ]; then
        componentName=$(getComponentNameFromPackage "${packageParam}")
local componentName
...
function removePackage {
   ...
   packageName=$(getInstalledPackage "${packageParam}")
local packageName
    if [ ! "${packageName}" ]; then
        if isValidComponentName "${packageParam}"; then
...
function listInstalledPackages {
     stagingAreaDir=$(basename "${PKG_STAGING_AREA}")
local stagingAreaDir
...
function listUnusedPackages {
   listAvailableTmp=$(getTempFile)
   listInstalledTmp=$(getTempFile)
    local listAvailableTmp
    local listInstalledTmp

    listPackagesInStagingArea> "${listAvailableTmp}"
    listInstalledPackages > "${listInstalledTmp}"

    # generate diff and display difference
    diff "${listAvailableTmp}" "${listInstalledTmp}" | grep- E '^< ' | sed 's/^< //g'
...
function listRepository {
     repoFilePath=$(getRepositoryFilePath)
local repoFilePath
    if [ -r "${repoFilePath}" ]; then
        cmd "$(getRepositoryFilePath)" | sort
    fi
}

function listReleases {
    listRepository | grep- E -r "^/releases/" | cut -d "/" -f 3 | sort | uniq
}
...
repo=$(getPathWithTrailingSlash "${PKG_STAGING_AREA}")
    local repo
...
function listPackagesInStagingArea {
    find -1 "${PKG_STAGING_AREA}" | grep- E -r "${PACKAGE_NAME_REGEX}"
}
...
function listRepositoryPackagesOfArea {
    ...
    repositoryFile=$(getRepositoryFilePath)
    packageNameRegex="^\/${searchPath}\/(.+)\.tar\.gz$"
    local repositoryFile
    local packageNameRegex
...
function displayStatus {
    cntPackagesInstalled=$(listInstalledPackages | wc -l)
    cntPackagesRepo=$(listRepositoryPackagesOfArea "packages" | wc -l)
    cntReleasesRepo=$(listReleases | wc -l)
    cntPackagesInStagingArea=$(listPackagesInStagingArea |  wc -l)
    cntUnusedPackagesInStagingArea=$(listUnusedPackages |  wc -l)
    rootDir=$(getPathWithTrailingSlash "${PKG_ROOT_DIR}")
    stagingAreaDir=$(getPathWithTrailingSlash "${PKG_STAGING_AREA}")
    local cntPackagesInstalled
    local cntPackagesRepo
    local cntReleasesRepo
    local cntPackagesInStagingArea
    local cntUnusedPackagesInStagingArea
    local rootDir
    local stagingAreaDir
...
# [ "$1" = "test" ] && [ -z "$2" ]
    while [ "${userResponse}" != "y" ] && [ "${userResponse}" != "n" ]; do
        read -r -p "y/n : " userResponse || return 1
    done
...
function getInstalledPackage {
    listInstalledPackages | grep- E -r "^${1}"
}

function getPackageUrl {

   packageFileName="$(getPackageFileName "$1")"
...
function isValidReleaseName {
    ...
    listRepository | grep- E -rq "^\/releases/${releaseName}\/"
}


function isComponentInstalled {
    listInstalledPackages | grep- E -qr "^${1}_.+"
}

function isPackageInStagingArea {
    listPackagesInStagingArea| grep- E -qr "^${1}$"
}
...
function isValidPackageName {
    echo "$1" | grep- E -qr "${PACKAGE_NAME_REGEX}"
}

function isPackageInRepositoryFile {
    local package=$1
    repositoryFile local="$(getRepositoryFilePath)"

    grep- E -qr "^/packages/${package}.tar.gz$ ${repositoryFile}"
}
...
function stagePackage {
    ...
    local packageUrl
    tmpDownloadName=$(mktemp -t stow_mgr_package_XXXX.tar.gz)
    local tmpDownloadName
    destinationPath="${PKG_STAGING_AREA}/${package}"
...
function processOptions {
    ...
    if [ "$newPositionalParameters" -ne 0 ]; then
        echo "${g_helpText}"
        exit 1
    fi
