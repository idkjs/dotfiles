#!/bin/bash
# https://github.com/shawnaxsom/dotfiles/blob/master/install.sh
...
create_symlink() {
  if [ $prompted_to_delete = false ]; then
    if [ ! -L "$2" ]; then
      if [ -f "$2" ] || [ -d "$2" ]; then
        read -r -p "Non-symbolic file already exists, okay to delete? (y/n)" yn
...
create_symlink   .yabairc                  ~/.yabairc
